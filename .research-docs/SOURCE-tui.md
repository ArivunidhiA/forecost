# Core Source: forecost/tui.py

Textual-based interactive dashboard with optional plotext charts.
...
Graceful degradation: if Textual not installed, `launch()` prints helpful message.

### Fallback Chart Rendering

```python
def _plotext_bar_chart(daily_costs):
    if not daily_costs or not HAS_PLOTEXT:
        return _text_bar_chart(daily_costs)
    try:
        # Plotext rendering
...
Always falls back to ASCII text bars if plotext fails.
...
```
┌─────────────────────────────────────────────┐
│  forecost -- Cost Forecast Dashboard        │
├─────────────────────┬───────────────────────┤
│  Projected Total:   │  Actual Spend: $X.XX  │
│  $XXX.XX            │  Remaining: $X.XX     │
│                     │  Tokens: X,XXX        │
│  [Bar Chart]        │  Burn Ratio: X.XXx    │
│                     │  Confidence: high     │
│                     │  Drift: on_track      │
│                     ├───────────────────────┤
│                     │  Model breakdown      │
...
- **q**: Quit application
- **r**: Refresh data (calls `on_refresh` callback, re-runs forecast)
...
|---------|----------|---------|
| textual | No (extra) | TUI framework |
| plotext | No (extra) | Terminal charts |

Install: `pip install forecost[tui]`
