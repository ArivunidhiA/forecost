# Core Source: forecost/tracker.py

Public tracking API for manual and decorator-based cost tracking.
...
for parent in [cwd, *cwd.parents]:
    toml_path = parent / ".forecost.toml"
    if toml_path.is_file():
        # Parse and resolve project path
```

Walks up directory tree from CWD looking for `.forecost.toml`. Caches result for 5 minutes to avoid repeated file system access.

### Session Stats Aggregation

In-memory thread-safe counters:
- `total_cost`: Running USD total
- `total_tokens`: Combined input+output tokens
- `calls`: Number of API calls
- `by_model`: Per-model breakdown (cost, tokens, calls)

Updated via `_record_usage()` with `_stats_lock` mutex.

### Dual Path: Session + Persistence

Every call updates:
1. In-memory `_session_stats` (fast, for `get_session_summary()`)
2. `WriteQueue` for SQLite persistence (async, batched)

---

## API Patterns

### Decorator: `@track_cost()`
...
@track_cost(provider="anthropic")
async def call_claude(prompt: str):
    return await client.messages.create(...)
```

Inspects return dict for `usage` key with OpenAI/Anthropic formats.

### Context Manager: `with track()`

```python
with track() as t:
    t.log_call("gpt-4o", tokens_in=1000, tokens_out=500)
    t.log_call("claude-3-opus", tokens_in=2000, tokens_out=1000)
```

Useful for batch operations or non-standard APIs.
...
- No `.forecost.toml` found: prints warning to stderr, doesn't crash
- `FORECOST_DISABLED=1`: silently returns without installing interceptor
- Missing `usage` key in decorated function: silently skips tracking
- Database unavailable: WriteQueue drops item (never blocks caller)
